...
parNew收集器在单核和超线程实现的伪双核上，都不一定比serial收集器效果更好，但是在cpu核数多了的情况下，还是很不错的

ParNew之所以是jdk7遗留系统之前的首先年轻代收集器，是因为只有他和serial才能和CMS收集器一起工作，CMS收集器只负责收集老年代
...
- 它基于标记复制算法实现垃圾收集，也是能并行收集的多线程收集器，
- 这个收集器，主要关注的是达到一个可控的吞吐量，而且上述的收集器是尽可能减少用户的停顿时间。
- 吞吐量=处理器用于用户代码的时候和处理器总消耗时间的比值
  ![image-20210119215214523](D:\my_study\my_study\picture\image-20210119215214523.png)
- 停顿时间越短就越适合需要与用户交互或需要保证服务响应质量的程序，良 好的响应速度能提升用户体验；而高吞吐量则可以最高效率地利用处理器资源，尽快完成程序的运算 任务，主要适合在后台运算而不需要太多交互的分析任务。
- parallel scavenger 提供了两个用于精确控制吞吐量的参数：-XX:MaxGCPauseMillis ->控制最大垃圾收集停顿时间，-XX:GCTimeRatio:直接设置吞吐量大小。
...
一次调优记录：https://www.cnblogs.com/lzmrex/articles/12878255.html

XX：+HeapDumpOnOutOfMemoryError参数，可以让虚拟机在内存溢出异常出现之后自动生成堆转储 快照文件
=======
-XX:UseConcMarkSweepGC 启动cms收集器
>>>>>>> 4adb71d26342a1f227d766a50d338cf370b8b552
